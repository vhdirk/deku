...
impl<'a, T, Ctx> DekuReader<'a, Ctx> for Mutex<T>
where
    T: DekuReader<'a, Ctx>,
    Ctx: Copy,
{
...
impl<'a, T, Ctx, Predicate> DekuReader<'a, (Limit<T, Predicate>, Ctx)> for Mutex<[T]>
where
    T: DekuReader<'a, Ctx>,
    Ctx: Copy,
    Predicate: FnMut(&T) -> bool,
{
    fn from_reader_with_ctx<R: Read + Seek>(
        ...
        // use Vec<T>'s implementation and convert to Mutex<[T]>
        let val = <Vec<T>>::from_reader_with_ctx(reader, (limit, inner_ctx))?;
        Ok(Mutex::from(val.into_boxed_slice()))
    }
}

impl<T, Ctx> DekuWriter<Ctx> for Mutex<[T]>
where
    T: DekuWriter<Ctx>,
    Ctx: Copy,
{
